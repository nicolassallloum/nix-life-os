...
NORMAL_EMAIL="${NORMAL_EMAIL:-step74.user@nixlifeos.com}"
...
RESULT_FILE="step74_authorization_regression_result_v2.log"
...
login_token() {
  ...
  curl -s -X POST "$BASE_URL/auth/login" \
    ...
    -d "{\"email\":\"$email\",\"password\":\"$password\"}" | jq -r '.data.token // .token // .access_token // empty'
}

{
print_header "STEP 74 — Protected Routes & Authorization Regression Test V2"
...
if [ -z "$USER_TOKEN" ]; then
  echo "Normal user does not exist or password changed. Registering with strong password..."
  curl -s -X POST "$BASE_URL/auth/register" \
    -H "Accept: application/json" \
    -H "Content-Type: application/json" \
    -d "{\"name\":\"$NORMAL_NAME\",\"email\":\"$NORMAL_EMAIL\",\"password\":\"$NORMAL_PASSWORD\",\"password_confirmation\":\"$NORMAL_PASSWORD\"}" | jq . || true

  USER_TOKEN="$(login_token "$NORMAL_EMAIL" "$NORMAL_PASSWORD")"
fi

if [ -z "$USER_TOKEN" ]; then
  echo "FAIL: Unable to create/login normal user. Check password policy or auth controller."
  exit 1
fi
