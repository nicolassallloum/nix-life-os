<template>
  <section class="productivity-page">
    ...
    <div v-else-if="dashboard?.summary?.empty_state" class="state-card empty-state">
      ...
      <p>
        Create productivity tasks, habits, goals, or calendar events to start seeing dashboard KPIs and charts. Zero-value cards are displayed below for validation.
      </p>
    </div>
...
const ChartBars = defineComponent({
  ...
  setup(props) {
    const maxValue = computed(() => {
      const values = (Array.isArray(props.items) ? props.items : []).map((item) => Number(item[props.valueKey] || 0));
    ...
    return () =>
      h(
        ...
        (Array.isArray(props.items) && props.items.length > 0
          ...
          : [h("div", { class: "mini-empty" }, "No chart data available yet.")])
      );
  },
});
...
const habitChartItems = computed(() => Array.isArray(dashboard.value?.charts?.habit_completion_last_7_days) ? dashboard.value.charts.habit_completion_last_7_days : []);
const calendarChartItems = computed(() => Array.isArray(dashboard.value?.charts?.calendar_next_7_days) ? dashboard.value.charts.calendar_next_7_days : []);
...
async function loadDashboard() {
  ...
  try {
    ...
    dashboard.value = {
      ...
      tasks: { ...fallback.tasks, ...(data.tasks || {}) },
      habits: { ...fallback.habits, ...(data.habits || {}) },
      goals: { ...fallback.goals, ...(data.goals || {}) },
      calendar: {
        ...fallback.calendar,
        ...(data.calendar || {}),
        next_events: Array.isArray(data.calendar?.next_events) ? data.calendar.next_events : [],
      },
      charts: {
        ...fallback.charts,
        ...(data.charts || {}),
        tasks_by_status: Array.isArray(data.charts?.tasks_by_status) ? data.charts.tasks_by_status : [],
        goals_by_status: Array.isArray(data.charts?.goals_by_status) ? data.charts.goals_by_status : [],
        habit_completion_last_7_days: Array.isArray(data.charts?.habit_completion_last_7_days) ? data.charts.habit_completion_last_7_days : [],
        calendar_next_7_days: Array.isArray(data.charts?.calendar_next_7_days) ? data.charts.calendar_next_7_days : [],
      },
    };
  } catch (err) {
