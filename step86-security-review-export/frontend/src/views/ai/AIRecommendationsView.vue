...
const latestScore = computed(() => (Array.isArray(dailyScores.value) ? dailyScores.value[0] : null) || null)
...
function normalizeSummary(payload) {
  const raw = payload?.data?.summary || payload?.summary || {}
  return {
    total: 0,
    pending: 0,
    viewed: 0,
...
async function loadRecommendations() {
  ...
  try {
    ...
    recommendations.value = normalizeList(response, ['recommendations'])
...
async function loadDailyScores(generateToday = false) {
  ...
  try {
    ...
    dailyScores.value = normalizeList(response, ['scores'])
