...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('ai_recommendations', function (Blueprint $table) {
            ...
            $table->index(
                ['user_id', 'status'],
                'idx_ai_recommendations_user_status'
            );

            $table->index(
                ['user_id', 'module'],
                'idx_ai_recommendations_user_module'
            );

            $table->index(
                ['user_id', 'severity', 'status'],
                'idx_ai_recommendations_user_severity_status'
            );

            $table->unique(
                ['user_id', 'duplicate_key'],
                'uq_ai_recommendations_user_duplicate_key'
            );
        });
...
};ai_recommendation_feedback_table