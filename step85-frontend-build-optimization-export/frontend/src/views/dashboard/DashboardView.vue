    ...
    <div v-else>
      ...
      <div class="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <EmptyState
          ...
          message="Your latest finance, health, project, productivity, and AI actions will appear here after records are created."
        />
      </div>
    </div>
  </section>
...
function logStep82Render(label, start) {
  if (!import.meta.env.DEV) return;

  requestAnimationFrame(() => {
    const duration = Math.round((step82Now() - start) * 100) / 100;
    console.info(`[STEP82] ${label} rendered in ${duration} ms`);
  });
}
...
function safeNumber(value) {
  ...
  if (!Number.isFinite(numberValue) || value === null || value === undefined) {
    return 0;
  }
...
function normalizeDashboardSummary(payload) {
  ...
  return {
    ...emptySummary(),
    ...data,
    total_balance: data.total_balance ?? finance.total_balance ?? finance.balance ?? 0,
    income: data.income ?? finance.income ?? finance.total_income ?? 0,
    monthly_expense: data.monthly_expense ?? finance.monthly_expense ?? finance.total_expense ?? 0,
    savings_rate: data.savings_rate ?? finance.savings_rate ?? 0,
    today_steps: data.today_steps ?? health.today_steps ?? health.steps_today ?? 0,
    today_calories: data.today_calories ?? health.today_calories ?? 0,
    water_intake_ml: data.water_intake_ml ?? health.water_intake_ml ?? health.water_today ?? 0,
    current_weight_kg: data.current_weight_kg ?? health.current_weight_kg ?? health.latest_weight ?? 0,
    ...
    total_tasks: data.total_tasks ?? productivity.total_tasks ?? 0,
  };
}
...
async function loadDashboard() {
  const step82Start = step82Now();
  loading.value = true;
  error.value = '';

  if (import.meta.env.DEV) console.time('[STEP82] Dashboard API Load');
  ...
  try {
    ...
    loading.value = false;
    if (import.meta.env.DEV) console.timeEnd('[STEP82] Dashboard API Load');
    logStep82Render('Dashboard', step82Start);
  }
}
